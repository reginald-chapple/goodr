...
class ProjectForm(forms.ModelForm):
    ...
    deadline = forms.DateField(required=True, widget=forms.DateInput(attrs={'class': 'form-control'}))
    cause = forms.ModelChoiceField(queryset=Cause.objects.all(), required=True, widget=forms.Select(attrs={'class': 'form-select'}))

    class Meta:
        model = Project
        fields = ("title", "description", "intended_impact", "target_amount", "location", "deadline", "cause",)
